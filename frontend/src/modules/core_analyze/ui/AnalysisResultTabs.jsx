...
const MetricBlock = ({ label, value, caption }) => (
  <div className="min-w-0 rounded-[24px] border border-white/10 bg-white/[0.08] p-4 text-white shadow-sm shadow-black/10 backdrop-blur">
    <p className="break-keep text-[11px] font-bold text-emerald-100/70">{label}</p>
    <p className="mt-2 text-3xl font-black tracking-tight text-white">{value}</p>
    {caption && <p className="mt-1 truncate text-xs font-bold text-slate-300/80">{caption}</p>}
  </div>
);
...
const HeroHeader = ({ reverseRiskResults, integratedItems }) => {
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-5 text-white shadow-[0_22px_60px_-34px_rgba(15,23,42,0.75)] md:p-7">
      <div className="pointer-events-none absolute -right-16 -top-20 h-52 w-52 rounded-[55%_45%_42%_58%_/_58%_37%_63%_42%] bg-emerald-400/15 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-24 left-1/3 h-44 w-44 rounded-[42%_58%_65%_35%_/_45%_54%_46%_55%] bg-cyan-300/10 blur-3xl" />
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-200/90">Contract Risk Report</p>
          <h3 className="mt-3 max-w-4xl break-keep text-2xl font-black leading-tight tracking-tight md:text-4xl">계약서의 위험 조항과 수정 방향을 정리했습니다</h3>
          <p className="mt-3 max-w-3xl break-keep text-sm font-semibold leading-7 text-slate-300">
            탐지된 독소조항을 기준으로 법적 근거, 연결 리스크, 누락된 보호 장치, 수정 가이드를 함께 검토할 수 있습니다.
          </p>
          {summary.topRisk && <p className="mt-4 inline-flex max-w-full rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1.5 text-sm font-bold text-emerald-100"><span className="truncate">우선 검토 항목: {summary.topRisk.title}</span></p>}
        </div>
...
const ClauseCard = ({ item, selected, onClick }) => {
  ...
  return (
    <button type="button" onClick={onClick} className={`w-full rounded-[24px] border bg-white p-4 text-left shadow-sm shadow-slate-200/50 transition duration-200 hover:-translate-y-[1px] hover:border-emerald-200 hover:shadow-[0_16px_38px_-30px_rgba(15,23,42,0.55)] ${selected ? style.selected : "border-slate-200"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-black uppercase tracking-[0.16em] text-emerald-600/70">검토 조항</p>
      ...
      <p className="mt-2 line-clamp-2 break-keep text-xs font-semibold leading-6 text-slate-500">{clause.originalText || clause.description || clause.reason || "상세 내용을 확인하세요."}</p>
...
const Section = ({ number, title, icon, children }) => (
  <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40">
    <div className="mb-4 flex items-center gap-2 text-sm font-black text-slate-900">
      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-950 text-[11px] text-white shadow-sm">{number}</span>
      {icon}
      {title}
    </div>
    {children}
...
const RiskScenarioCard = ({ risk }) => {
  ...
  return (
    <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-slate-50/80 shadow-sm shadow-slate-200/40">
      <button type="button" onClick={() => setOpen((prev) => !prev)} className="flex w-full items-start justify-between gap-4 px-4 py-3.5 text-left transition hover:bg-emerald-50/60">
        <div className="min-w-0">
      ...
      {open && (
        <div className="space-y-3 border-t border-slate-200 bg-white p-4 text-xs font-semibold leading-6 text-slate-600">
          ...
          {risk.suggestion && (
            <div className="rounded-[20px] border border-emerald-100 bg-emerald-50/80 p-3 text-emerald-900">
...
const DetailPanel = ({ item, onClose }) => {
  ...
  return (
    <div className="sticky top-28 space-y-4 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_50px_-34px_rgba(15,23,42,0.55)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
      ...
      <Section number="03" title="추천 수정 가이드" icon={<Wrench size={16} className="text-purple-500" />}>
        <div className="rounded-[22px] border border-emerald-100 bg-emerald-50/80 p-4 text-sm font-bold leading-7 text-emerald-900">
...
const AiRawView = ({ apiAnalysisResult }) => {
  ...
  return (
    <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/40">
      ...
      {aiText ? <pre className="mt-4 max-h-[540px] overflow-auto rounded-[24px] border border-slate-200 bg-slate-50 p-4 text-xs font-semibold leading-7 text-slate-700 shadow-inner shadow-slate-200/40">{aiText}</pre> : <p className="mt-4 text-sm font-semibold text-slate-500">아직 AI 분석 결과가 없습니다.</p>}
    </div>
  );
};
...
const SuggestionsView = ({ integratedItems }) => {
  ...
  return (
    <div className="space-y-4 rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_14px_42px_-32px_rgba(15,23,42,0.5)]">
      <div>
        <h3 className="text-lg font-black text-slate-900">수정 가이드</h3>
        <p className="mt-1 break-keep text-sm font-semibold leading-6 text-slate-500">탐지된 조항별로 수정 방향과 보완 문구를 Risk Board와 같은 톤으로 정리했습니다.</p>
      </div>
      {suggestions.map((item, index) => (
        <div key={item.id} className="rounded-[24px] border border-emerald-100 bg-emerald-50/70 p-4 shadow-sm shadow-emerald-100/40">
          ...
          <p className="mt-3 break-keep text-sm font-bold leading-7 text-emerald-900">{item.suggestion}</p>
        </div>
      ))}
    </div>
  );
...
const AnalysisResultTabs = ({ contractText = "", apiAnalysisResult = null }) => {
  ...
  return (
    <div className="space-y-5 rounded-[32px] bg-slate-50/70">
      <HeroHeader reverseRiskResults={reverseRiskResults} integratedItems={integratedItems} />
      <div className="rounded-[24px] border border-slate-200 bg-white/95 p-2 shadow-[0_12px_34px_-28px_rgba(15,23,42,0.45)]">
        <div className="grid grid-cols-3 gap-2">
          {tabs.map((tab) => (
            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`flex min-w-0 items-center justify-center gap-1.5 rounded-2xl px-3 py-2.5 text-sm font-black transition duration-200 ${activeTab === tab.id ? "bg-slate-950 text-white shadow-[0_12px_28px_-20px_rgba(15,23,42,0.8)]" : "text-slate-500 hover:bg-emerald-50 hover:text-emerald-700"}`}>
              {tab.icon}
              {tab.label}
            </button>
          ))}
