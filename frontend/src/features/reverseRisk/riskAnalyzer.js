...
const toEvidenceItems = (risk) => {
  const evidenceSentences = Array.isArray(risk.evidenceSentences) ? risk.evidenceSentences : [];
...
export function analyzeContractRisk(contractText, selectedScenarioIds = [], apiAnalysisResult = null) {
  const analysis = analyzeReverseRisk(contractText, apiAnalysisResult);
  const selectedIds = Array.isArray(selectedScenarioIds) ? selectedScenarioIds : [];

  return analysis.results
    .filter((risk) => selectedIds.length === 0 || selectedIds.includes(risk.id))
    .map((risk) => {
      const evidenceItems = toEvidenceItems(risk);
      const evidenceFound = evidenceItems.length > 0 || risk.matchedKeywords.length > 0;

      return {
        ...risk,
        evidenceFound,
        evidenceItems,
        hasContractEvidence: evidenceItems.length > 0,
        hasToxicClauseEvidence: false,
